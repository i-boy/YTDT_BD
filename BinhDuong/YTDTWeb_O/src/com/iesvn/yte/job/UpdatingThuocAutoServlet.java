...
public class UpdatingThuocAutoServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		// 20120702 bao.ttc: neu set UPDATING_THUOC_HOUR = NO thi ko chay Updating Thuoc AutoServlet
		if (IConstantsRes.UPDATING_THUOC_HOUR.equals("NO")) {
			log.info("Not run Updating Thuoc AutoServlet");
			return;
		}
		
		log.info("Initializing Scheduler PlugIn for Updating Thuoc Jobs!");
		super.init(config);
		...
		try { 
			...
			if (startOnLoad == null || (Boolean.getBoolean(startOnLoad))){
				...
				JobDetail job = newJob(UpdatingThuocJob.class)
			    						.withIdentity("UpdatingThuocJob", "UPDATINGTHUOC_GROUP")
			    						.storeDurably()
			    						.requestRecovery()
			    						.build();
				
				String cron_HrsExpr = IConstantsRes.UPDATING_THUOC_HOUR;
				String cron_MinuteExpr = IConstantsRes.UPDATING_THUOC_MINUTE;
				String cron_SecExpr = IConstantsRes.UPDATING_THUOC_SEC;
				String cronExpr = cron_SecExpr + " " + cron_MinuteExpr +" " + cron_HrsExpr+ " * * ?";
				CronTrigger trigger = newTrigger()
	            						.withIdentity("UpdatingThuocTrigger", "UPDATINGTHUOC_GROUP")
	            						.withSchedule(cronSchedule(cronExpr))
	            						.build();
					            
				scheduler.scheduleJob(job, trigger);
				log.info("Updating Thuoc Job scheduled now ..");
				scheduler.start();
