 ...
 * @author DOXP
 */
public class SendPhieuNhapKhoAction extends Action {
			...
			if (listPNK.getLength() > 0) {
				...
				for (int i = 0; i < listPNK.getLength(); i++) {
					...
					if (listCTPN.getLength() > 0) {
						...
						ArrayList<CtNhapKho> listCTNK = new ArrayList<CtNhapKho>();
						ArrayList<TonKho> listTk = new ArrayList<TonKho>();
						for (int j = 0; j < listCTPN.getLength(); j++) {
							...
							try {
								...
								if (tk != null) {
									...
									ctnk.setCtnhapkhoThutu(Integer.valueOf(j) + 1);
									ctnk.setCtnhapkhoDongia(dg);
	...
	public CtNhapKho getTonKho(Node nodeTK, TonKho tk) throws ServiceException, RemoteException, Exception {
		logger.debug("-----getTonKho()-----");
		logger.debug(String.format("-----nodeTK: %s", nodeTK));
		...
		if (nodeTK != null) {
			...
			//String tkMa = "" + nodeTK.getAttributes().getNamedItem("TONKHO_MA").getTextContent();
			...
			if (!quyCach.equals("") && !quyCach.equalsIgnoreCase("null")) {
				logger.debug(String.format("-----quy cach: %s", quyCach));
			...
			if (!soLuong.equals("") && !soLuong.equalsIgnoreCase("null")) {
				logger.debug(String.format("-----so luong: %s", soLuong));
			...
			if (!dmtMa.equals("") && !dmtMa.equalsIgnoreCase("null")) {
				try {
					...
					if (dmt != null) {
						logger.debug(String.format("-----dm thuoc ma: %s", dmt.getDmthuocMa()));
			...
			if (!quocGiaMa.equals("") && !quocGiaMa.equalsIgnoreCase("null")) {
				try {
					...
					if (qg != null) {
						logger.debug(String.format("-----quoc gia: %s", qg.getDmquocgiaMa()));
			...
			if (!hangsxMa.equals("") && !hangsxMa.equalsIgnoreCase("null")) {
				try {
					...
					if (hangsx != null) {
						logger.debug(String.format("-----hang sx: %s", hangsx.getDmnhasanxuatMa()));
			...
			if (!tonKhoNaN.equals("") && !tonKhoNaN.equalsIgnoreCase("null")) {
				logger.debug(String.format("-----nam nhap: %s", tonKhoNaN));
			...
			if (!tonKhoDG.equals("") && !tonKhoDG.equalsIgnoreCase("null")) {
				logger.debug(String.format("-----don gia: %s", tonKhoDG));
				tk.setTonkhoDongia(new Double(tonKhoDG));
				ctnk.setCtnhapkhoDongia(new Double(tonKhoDG));
			}
				
			if (!tonKhoDGB.equals("") && !tonKhoDGB.equalsIgnoreCase("null")) {
				logger.debug(String.format("-----don gia ban: %s", tonKhoDGB));
	...
	public PhieuNhapKho getPhieuNhap(Node nodePN) throws ServiceException,
		...
		if (nodePN != null) {
			...
			if (!nvTL.equals("") && !nvTL.equalsIgnoreCase("null")) {
				try {
					...
					if (nv != null) {
						logger.debug("nhan vien: "
								+ nv.getDtdmnhanvienTen());
			...
			if (!nhanVienCN.equals("") && !nhanVienCN.equalsIgnoreCase("null")) {
				try {
					...
					if (nv != null) {
						logger.debug(String.format("-----nhan vien cap nhat: %s", nv.getDtdmnhanvienMa()));
