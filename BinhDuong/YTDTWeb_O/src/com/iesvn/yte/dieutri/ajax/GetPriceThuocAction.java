...
public class GetPriceThuocAction extends Action {
	public String performAction(String request) throws Exception {
		StringBuffer buf = new StringBuffer();
		List objDmTonKho = null;
		
		String mathuoc = request.substring(0,request.indexOf("___")); // 3_
		...
		if (objDmTonKho != null) {
			 for (Object obj : objDmTonKho){
				 ...
				 buf.append("<record MaSo='" + tonKho.getTonkhoMa() +  "' NamNhap='"								 
							 + NamNhap + "' NgayHanDung='"	
							 + NgayHanDung + "' ThangHanDung='"
							 + ThangHanDung + "' NamHanDung='"
							 + NamHanDung  + "' DonGia='"
							+ Utils.formatNumber(donGia, "##.####")  + "' DonGiaBH='"					
							+ Utils.formatNumber(donGiaBH, "##.####") + "' DonGiaMP='"						
							+ mienPhi + "' DonGiaYC='"
							+ YC + "' NDM='" 
							+ ndm + "' KhongThu='" 
							...
							+ maSoQG + "' QG='" 
							+ maQG + "' NCT='" 							
							+ tonKho.getDmnctMaso().getDmnctMaso() + "' NCT_Ma='" 
							+ tonKho.getDmnctMaso().getDmnctMa() + "' NKP='" 
							+  tonKho.getDmnguonkinhphiMaso().getDmnguonkinhphiMaso() + "' NKP_Ma='" 	
							+  tonKho.getDmnguonkinhphiMaso().getDmnguonkinhphiMa() + "' MaLK='" 	
							+  tonKho.getTonkhoMalienket() + "' Ton='" 
							+ Utils.formatNumber(ton.doubleValue(), "##.##") + "' NgayChinhSua='" 
							+ "" + "' DT='" 
							+ "" + "' HamLuong='" 
							+ hamluong + "' Dvt='" 
							+ dvt + "' tenThuoc='" 
							+ tenThuoc + "' tenQG='" + tenQG 
							+ "' tenHSX='" + tenHSX 
							+ "' hanDung='" + hd + "' />");						 
			 }			   				
		}
