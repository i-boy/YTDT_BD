 ...
 * @author DOXP
 */
public class SendThuocNoiTruAction extends Action {
	@Override
	public String performAction(String request) {
		...
		if (xmlDoc != null) {
			...
			if (listTNT.getLength() > 0) {
				System.out.println("Thuoc noi tru count "
						+ listTNT.getLength());
				String maPhu = null;
				ArrayList<ThuocNoiTru> listThuoc = new ArrayList<ThuocNoiTru>();
				for (int i = 0; i < listTNT.getLength(); i++) {
					myBreak = 0;
					Node nodeTNT = listTNT.item(i);
					maPhu = nodeTNT.getAttributes().getNamedItem("THUOC_NOI_TRU_MAPHU1")
							.getTextContent();
					ThuocNoiTru tnt = null;
					try {
						tnt = this.getThuocNoiTru(nodeTNT);
						if (tnt != null) {
							listThuoc.add(tnt);
					...
					if (myBreak == 0) {
						try {
							...
							String tntID = "";//tntWS.createThuocNoiTru(listThuoc, new ArrayList<ThuocNoiTru>());
							System.out.println("insert tiep don & benh nhan "
									+ tntID);
							if (!("".equals(tntID)))
								okId += maPhu + "---";
							else
	...
	public ThuocNoiTru getThuocNoiTru(Node nodeTNT) throws ServiceException,
		...
		if (nodeTNT != null) {
			...
			if (tutruc_pdt != null && !tutruc_pdt.equals("")){
				if (tutruc_pdt.equals("0")){
					tnt.setThuocnoitruTutrucPdt(0);
				}else if (tutruc_pdt.equals("1")){
					tnt.setThuocnoitruTutrucPdt(1);
				}else{
