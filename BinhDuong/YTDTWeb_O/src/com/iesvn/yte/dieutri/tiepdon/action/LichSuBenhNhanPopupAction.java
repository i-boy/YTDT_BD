...
@Scope(SESSION)
@Name("danhsachlichsubenhnhan")
@Synchronized(timeout = 6000000)
public class LichSuBenhNhanPopupAction implements Serializable{

	/**
	 * 
	 */
	private static final long serialVersionUID = -493331384169670628L;
	private static Logger log = Logger.getLogger(LichSuBenhNhanPopupAction.class);	
	private String result;

	@DataModel
	private List<ItemForSearch> listFordanhsachlichsubenhnhan;
	
	
	public List<ItemForSearch> getListForDanhsachbenhnhan() {
		return listFordanhsachlichsubenhnhan;
	}

	public void setListForDanhsachbenhnhan(
			List<ItemForSearch> listForDanhsachbenhnhan) {
		this.listFordanhsachlichsubenhnhan = listForDanhsachbenhnhan;
	}

	@Create
	public void init() {
		log.info("---init");
		listFordanhsachlichsubenhnhan = new ArrayList<ItemForSearch>();		
	}
		...
		if (result!=null & !result.equals("")){
			...
			try {
				...
				for (int i = 0; i < arr.length(); i++){
					...
					listFordanhsachlichsubenhnhan.add(item);
				}
			} catch (Exception e) {} 
		}	
	}
