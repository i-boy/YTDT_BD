...
public class DangKyKhamBenhCapCuuAction implements Serializable {
	...
	public void ghiNhan() {
		...
		// 1: CÃ¹ng tuyáº¿n - nÆ¡i DK KCB trÃ¹ng vá»›i mÃ£ bá»‡nh viá»‡n khÃ¡m.
		// 2: KhÃ¡c tuyáº¿n - BH cÃ¹ng tá»‰nh khÃ¡c nÆ¡i DK KCB.
		// 3: Tuyáº¿n khÃ¡c - BH khÃ¡c tá»‰nh vá»›i nhau (default: tÆ°Æ¡ng tá»± form Ä�K khÃ¡m B111)
		if (tiepdon.getDoituongMa(true).getDmdoituongMa().equals("BH")){
		...
		if( qrCode_Input.trim().length() > 50 && qrCode_Input.endsWith("$") ) {
			...
			if( outputArr[0] != null && !outputArr[0].equals("") ) {
				if( outputArr[0].length() == 15 ) {
					...
					checkSoTheBHYT();
					tiepdon.setTiepdonSothebh(outputArr[0].toUpperCase());
					tiepdon.getDoituongMa(true).setDmdoituongMa("BH");
			...
			if( outputArr[1] != null && !outputArr[1].equals("") ) {
				tiepdon.getBenhnhanMa(true).setBenhnhanHoten(outputArr[1].toUpperCase());
			}

			// NGAY - NAM SINH - TUOI
			tiepdon.getBenhnhanMa(true).setBenhnhanDonvituoi(new Short("1"));
			if (outputArr[2] != null && !outputArr[2].equals("")) {
				if (outputArr[2].length() == 4) {
					tiepdon.getBenhnhanMa(true).setBenhnhanNamsinh(outputArr[2]);
					...
					tiepdon.getBenhnhanMa(true).setBenhnhanNamsinh(outputArr[2].substring(6));
				}
			}

			// GIOI TINH
			if( outputArr[3] != null && !outputArr[3].equals("") ) {
				gioi = outputArr[3].equals("1") ? "true" : "false";
			}

			// DIA CHI
			if( outputArr[4] != null && !outputArr[4].equals("") ) {
				//tiepdon.getBenhnhanMa(true).setBenhnhanDiachi(outputArr[4]);
			...
			if( outputArr[5] != null && !outputArr[5].equals("") ) {
				...
				if( kcbbdStr.length() == 6 && kcbbdStr.contains(".") ) {
					...
					if(tinhBhyt == null) {
						tiepdon.setTinhbhytMa(new DmTinh());
						//tiepdon.getBenhnhanMa(true).setTinhMa(new DmTinh());
					} else {
						tiepdon.setTinhbhytMa(tinhBhyt);
						tiepdon.getBenhnhanMa(true).setTinhMa(tinhBhyt);
					}
				}
			}
