...
import com.iesvn.yte.dieutri.delegate.HsbaPhieuGuiXacDelegate;
...
@Scope(CONVERSATION)
@Name("B291_Phieuguixac")
@Synchronized(timeout = 6000000)
public class PhieuGuiXacAction implements Serializable{

	private static final long serialVersionUID = -5106695460338085910L;
	private static Logger log = Logger.getLogger(PhieuGuiXacAction.class);
	
	@In(required = false)
	@Out(required = false)
	Identity identity;
	private HsbaPhieuGuiXac pgx;
	...
	public void displayHSBA(){
		...
		String maHsba = pgx.getHsbaSovaovien().getHsbaSovaovien().trim();
		Hsba hsba_tmp=null;
		if (!maHsba.equals("")){
	...
	public void displayPhieuGuiXac(){
		log.info("---displayPhieuGuiXac");
		String maPgx = pgx.getHsbapgxMa().trim();
		HsbaPhieuGuiXac pgx_tmp = null;
		if (!maPgx.equals("")){
			pgx_tmp = HsbaPhieuGuiXacDelegate.getInstance().findByHsbapgxMa(maPgx);
			if (pgx_tmp==null){
				nofound = "true";
				crrDate= ngayCap = new SimpleDateFormat("dd/MM/yyyy").format(new Date());
				pgx_tmp = new HsbaPhieuGuiXac();
				FacesMessages.instance().add(IConstantsRes.PGX_NULL, maPgx);
				
				
			}	
			pgx = pgx_tmp;
			setNgayVV(formatDate(pgx.getHsbaSovaovien(true).getHsbaNgaygiovaov()));
			setGioVV(formatDateTime(pgx.getHsbaSovaovien(true).getHsbaNgaygiovaov()));
			setChuandoanMa(pgx.getHsbaSovaovien(true).getHsbaTuvong(true).getDmbenhicdMa());
			if(pgx.getHsbaSovaovien(true).getHsbaTuvong(true).getDmbenhicdMaso()!=null){
				setChuandoanMaso(pgx.getHsbaSovaovien(true).getHsbaTuvong(true).getDmbenhicdMaso());
			}
			
			setChuandoanName(pgx.getHsbaSovaovien(true).getHsbaTuvong(true).getDmbenhicdTen());
	...
	public String getdmBenhIcdMaCX(){
		return pgx.getHsbaSovaovien(true).getHsbaTuvong(true).getDmbenhicdMa();
	}
	
	public String getdmBenhIcdTenCX(){
		return pgx.getHsbaSovaovien(true).getHsbaTuvong(true).getDmbenhicdTen();
	}
	...
	public void ghiNhan(){
		...
		if (!ngayCap.trim().equals("")){
			pgx.setHsbapgxNgaygiocn(Utils.getDBDate("00:00", ngayCap).getTime());
		}
//		if (!ngayCX.trim().equals("")){
//			if(gioCX.trim().equals("")){
//				pgx.setHsbapgxNgaycx(Utils.getDBDate("00:00", ngayCX).getTime());
//			}else
//			{
//				pgx.setHsbapgxNgaycx(Utils.getDBDate(gioCX, ngayCX).getTime());
//			}
//		}
		if (!ngayVV.trim().equals("")){
			if(gioVV.trim().equals("")){
				pgx.setHsbapgxNgayvaovien(Utils.getDBDate("00:00", ngayVV).getTime());
			}else
			{
				pgx.setHsbapgxNgayvaovien(Utils.getDBDate(gioVV, ngayVV).getTime());
			}
		}
		pgx.setHsbapgxNgaygiocn(new Date());
		pgx.setNhanvienMa(DtDmNhanVienDelegate.getInstance().findByNd(identity.getUsername()));
...
//			dmbTmp = (DmBenhIcd)dtutilDelegate.findByMaSo(getChuandoanMaso(), "DmBenhIcd", "dmbenhicdMaso");
			...
			pgx.setHsbapgxChuandoanma(dmbTmp);
		} catch (Exception ex) {
			System.out.println("Loi tai "+ex.toString());
		}
		if (isUpdate){
			result = HsbaPhieuGuiXacDelegate.getInstance().update(pgx);
			if (result == null || result.equals("")){
				nosuccess = "true";
				FacesMessages.instance().add(IConstantsRes.PGX_CN_THATBAI);
			}else{
				pgx.setHsbapgxMa(result);			
				FacesMessages.instance().add(IConstantsRes.PGX_CN_THANHCONG, result);
			}
		}else{
			result = HsbaPhieuGuiXacDelegate.getInstance().insert(pgx);
			if (result == null || result.equals("")){
				nosuccess = "true";
				FacesMessages.instance().add(IConstantsRes.PGX_LT_THATBAI);
			}else{
				pgx.setHsbapgxMa(result);			
				FacesMessages.instance().add(IConstantsRes.PGX_LT_THANHCONG, result);
			}
		}
	}
	...
	public void XuatReport() {
		loaiBCDT="PhieuGuiXac";
		log.info("Vao Method XuatReport giay chuyen xac");
		String baocao1=null;
		String pathTemplate=null;
		try {
			pathTemplate = IConstantsRes.PATH_BASE+IConstantsRes.PATH_REPORT_TEMPLATE_DIEU_TRI+"hsba/Phieuguixac.jrxml";
			...
			params.put("khoaRav", pgx.getHsbaSovaovien(true).getHsbaKhoarav(true).getDmkhoaTen());
			//log.info(String.format("-----khoa ra vien: %s", params.get("khoaRav")));
			params.put("GIAMDOC", IConstantsRes.REPORT_DIEUTRI_GIAMDOC_BV);
			
			params.put("magiay", pgx.getHsbapgxMa());
			log.info(pgx.getHsbapgxMa());
			
			// 20101111 bao.ttc: them cac thong tin khac
			if(pgx.getHsbaSovaovien() != null){
				
				Hsba hsba = pgx.getHsbaSovaovien();
				String diachistr="";
			...
			Class.forName("oracle.jdbc.OracleDriver");
			    ...
			    baocao1=XuatReportUtil.ReportUtil(jasperPrintDT,index,IConstantsRes.PATH_BASE+IConstantsRes.PATH_REPORT_RESULT_DIEU_TRI+"hsba/", "pdf","PhieuGuiXac");
	...
	public HsbaPhieuGuiXac getPgx() {
		return pgx;
	}

	public void setPgx(HsbaPhieuGuiXac pgx) {
		this.pgx = pgx;
	}
