...
@Scope(CONVERSATION)
@Name("B4122_Xuathangtheophieudt")
@Synchronized(timeout = 6000000)
public class XuatHangTheoPhieuDuTruAction implements Serializable {
	...
	private static Logger log = Logger
			.getLogger(XuatHangTheoPhieuDuTruAction.class);
	...
	@DataModel
	private List<CtXuatKho> listCtxk = new ArrayList<CtXuatKho>();
	...
	public String init(String loaikho) throws Exception {
		...
		if (listDmLoaiThuocs.size() == 0 || hmLoaiThuoc.size() == 0 ) {
			refreshDmLoaiThuoc();
			//log.info("########## refreshDmLoaiThuoc ##########");
		}
	...
	private void updateRow() {
		...
		int i = listCtxk.indexOf(nhapctSelected_B4122);
		if (i < 0) {
			insertRow();
		}
		//log.info("****i=" + i + "******");
		listCtxk.set(i, nhapctSelected_B4122);
		Caculation(listCtxk);
		updateNhapct = false;
	}
	...
	public void delete(int index) throws Exception {
		...
		//log.info("listCtxk.indexOf(nhapctSelected):"+listCtxk.indexOf(nhapctSelected_B4122));
		
		listCtxk.remove(index);
		Caculation(listCtxk);
		resetValue();
		updateNhapct = false;
		maFinish = "";
	...
	public void ghinhan() throws Exception {
		...
		if (ngayXuat != null && !ngayXuat.trim().equals("")) {
			...
			phieuXuatKho.setPhieuxuatkhoNgaylap(new Date());
		}
			
		for (CtXuatKho ctxk : listCtxk) {
		...
		maFinish = pxkWS.XuatPhieuDuTru(listCtxk, phieuXuatKho, IConstantsRes.PRIORITY_TON_LO_THUOC);
		...
		//log.info("AAA -- hienThiGhiNhan: " + hienThiGhiNhan +"hienThiInPhieu: " + hienThiInPhieu );	
		if(maFinish != null && !maFinish.equals("")){
			//log.info("maFinish:"+maFinish);
			if (maFinish.indexOf(".") >= 0) {
				setHienThiGhiNhan(false);
				setHienThiInPhieu(false);
				FacesMessages.instance().add(maFinish);
			} else {
				setHienThiGhiNhan(false);
				setHienThiInPhieu(true);
				FacesMessages.instance().add(IConstantsRes.SUCCESS);				
				maPhieuXuat  = maFinish;
				displayPhieuXuatKho();
			}
			
			//log.info("BBB -- hienThiGhiNhan: " + hienThiGhiNhan +"hienThiInPhieu: " + hienThiInPhieu );
			// Luu log he thong
	        yteLog.setForm("B4122_Xuathangtheophieudt");
		...
		//log.info("CCC -- hienThiGhiNhan: " + hienThiGhiNhan +"hienThiInPhieu: " + hienThiInPhieu );
	...
	public void nhaplai() throws Exception {
		...
		init("");
		listCtxk = new ArrayList<CtXuatKho>();
		tongTien = "";
		maPhieuDuTru = "";
		maPhieuXuat="";
		this.maFinish = "";
	...
	public void displayPhieuXuatKho() throws Exception{
		...
		listCtxk = ctxkDel.findByphieuxuatkhoMa(maPhieuXuat);
		Caculation(listCtxk);
	...
	//Luu y: neu ma phieu du tru nay da ton tai trong bang phieu xuat kho thi cho phep hien thi phieu xuat kho len
	...
	public void displayInfor() throws Exception {
		try {
			...
			PhieuXuatKhoDelegate pxkDel = PhieuXuatKhoDelegate.getInstance();
			CtXuatKhoDelegate ctxkDel = CtXuatKhoDelegate.getInstance();
			PhieuXuatKho pxkho = pxkDel.findByPhieuDuTruAndKhoXuat(maPhieuDuTru, dmkhoaXuat.getDmkhoaMaso());
			if(pxkho != null){
				phieuXuatKho = pxkho;
				SetInforUtil.setInforIfNullForPhieuXuatKho(phieuXuatKho);
				setMaPhieuXuat(phieuXuatKho.getPhieuxuatkhoMa());
				...
				SimpleDateFormat frmtDate = new SimpleDateFormat("dd/MM/yyyy");		
				setNgayXuat(frmtDate.format(phieuXuatKho.getPhieuxuatkhoNgaylap()));
				listCtxk = ctxkDel.findByphieuxuatkhoMa(phieuXuatKho.getPhieuxuatkhoMa());
				Caculation(listCtxk);
				maFinish =phieuXuatKho.getPhieuxuatkhoMa();
			...
			phieuDt=ctpdtWS.findByPhieuDuTruKhoXuatPhanBiet(maPhieuDuTru, CtPhieuDtDelegate.LANH_THUOC_KHOA_PHONG, dmkhoaXuat.getDmkhoaMaso());
			List<CtPhieuDt> ctpdt_tmp = ctpdtWS.findByPhieuDuTruMa(maPhieuDuTru);
			
			if (ctpdt_tmp == null || ctpdt_tmp.size() == 0 || phieuDt==null || phieuDt.equals(null)) {
				...
				//log.info("Co thong tin phieu du tru" + ctpdt_tmp);
				phieuXuatKho = new PhieuXuatKho();
			 ...
			 CtPhieuDtDelegate ctPhieuDtDel = CtPhieuDtDelegate.getInstance();
				boolean daXuatThuoc = ctPhieuDtDel.daXuatThuocTheoPhieuDT_New(maPhieuDuTru);
				if (daXuatThuoc == true){
					FacesMessages.instance().add(IConstantsRes.PHIEU_DU_TRU_DA_XUAT_HANG,maPhieuDuTru);
					hienThiGhiNhan=false;
			...
			//log.info("error on function displayInfor");
		}
		//log.info("maFinish : " + maFinish);
	...
	public void XuatReport() {
		reportTypeKC="B4122_XuatHangTheoPhieuDuTru";
		//log.info("Vao Method XuatReport B4122_XuatHangTheoPhieuDuTru");
		String baocao1=null;
...
//			log.info("Ma phieu :" + maFinish);
	...
	public static void setLog(Logger log) {
		XuatHangTheoPhieuDuTruAction.log = log;
	}
