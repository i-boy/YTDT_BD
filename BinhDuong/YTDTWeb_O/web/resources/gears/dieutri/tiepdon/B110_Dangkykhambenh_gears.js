...
function focusToKcbBhyt(){
	...
	document.getElementById(prefix_component + "KCBBHYT_MA").select();
}
...
function setCurrentHourDateFromGetTDFromBHYT(){
	    document.getElementById(prefix_component + "NGAYTG").value = getDateSystem_dd_MM_yyyy();
...
function temp() {
	document.getElementById(prefix_component + "TIEPDON_NGAYGIO").value = document.getElementById(prefix_component + "NGAYTG").value + " " + document.getElementById(prefix_component + "GIO").value;
}
...
function LoadDB_formMaPhuTD(myMaPhu){
	...
	for(var i = 0; i < listTd.length; i++) {
		...
		if ( ngaygio != null ) {
			document.getElementById(prefix_component + "NGAYTG").value = ngaygio.substr(0,10);
...
function onSubmit() {
	try{			
		...
		if (bSave){
    		...
    		if (valid == true){
    			valid = (iesvn_ValidateIntRange(form) && iesvn_ValidateComparedDates(form)); 
			...
			if(valid) {
				var dtHienTai = document.getElementById(prefix_component + "doituongHientai").value;
				var dtMoi = document.getElementById(prefix_component + "DOITUONG_MA").value;
				if (dtHienTai != ''  && dtHienTai != dtMoi) {
					if (dtMoi == 'TP') {
						alert("Lưu ý : Phải thu tiền khám của bệnh nhân.");
					} else if (dtHienTai == 'TP'){
						alert("Lưu ý : Trả lại tiền khám cho bệnh nhân.");
					}
				}
			}
    	} 	    	    
...
function testBHYTTrungTrongNgay(ghinhan){ // ghi nhan == true or false
    ...
    if (doituong == 'BH'){
		...
		if (sothebhyt != null && sothebhyt != ''){
			var ngaytd = document.getElementById(prefix_component + "NGAYTG").value;
...
//##############################
function loadTiepdonComplete(){
	var maTD = document.getElementById(prefix_component + "TIEPDON_MA").value;
	if (maTD =='') {
		setValueOnLoad();
	}	
	...
	myOnblurTextbox(prefix_component +'KHOIBHYT_MA','DT_DM_KHOI_BHYT');	
	myOnblurTextboxTinhBHYT(prefix_component +'TINHBHYT_MA','DM_TINH__2');
	myOnblurTextboxDmNoiDKKCBBD(prefix_component + 'KCBBHYT_MA','DM_BENH_VIEN',document.getElementById(prefix_component+'TINHBHYT_MA').value);	
	myOnblurTextbox(prefix_component +'KCBBHYT_MA','DM_BENH_VIEN');
	
	loadBenhNhanComplete();
}
function loadBenhNhanComplete(){
	myOnblurTextbox(prefix_component +'DANTOC_MA','DM_DAN_TOC');
	myOnblurTextbox(prefix_component +'TINH_MA','DM_TINH');
	myOnblurTextbox(prefix_component +'HUYEN_MA','DM_HUYEN');
	myOnblurTextbox(prefix_component +'XA_MA','DM_XA');
...
function checkHosoDaTT() {
	var daTT = document.getElementById(prefix_component + "__hosoDaTT").value;
	if(daTT == "true") {      		      		
  		document.getElementById("div_ghinhan").style.display = "none";
  		document.getElementById("div_huykham").style.display = "none";  		
    } else {    	
