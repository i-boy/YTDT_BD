function init() {
	if (window.google && google.gears) {
		//try {
            ...
            //setAttrForCombobox(prefix_component + 'DT_DM_NHAN_VIEN_MA','DT_DM_NHAN_VIEN_span', 'DT_DM_NHAN_VIEN','getDtDmNhanVienByMaKhoa_ThuNgan()',"","","");
            //alert('135');
...
function setDefaultValueForNVPhat(){
  if (document.getElementById(prefix_component + "DT_DM_NHAN_VIEN_MA").value == null || document.getElementById(prefix_component + "DT_DM_NHAN_VIEN_MA").value ==""){
  	var nvPhatClientDefault = DtDmClientDefault.getByFieldValue("Ma", "B3232_ThanhToanBenhNhanBHYT_nvphat");
...
function luuTruGiaTriClientDefault(){
		...
		if (giaTriNVPhat != null && giaTriNVPhat != ""){
			...
			if (nvPhatClientDefault == null || nvPhatClientDefault == false) { //insert
				...
				chValues[0] = 431101;
				chValues[1] = "B3232_ThanhToanBenhNhanBHYT_nvphat";
				...
				updateObjectWithTrueValue(DtDmClientDefault,"Ma","B3232_ThanhToanBenhNhanBHYT_nvphat", chNames,chValues );
...
function maPhieuOnComplete(){
	...
	if (document.getElementById(prefix_component + "nofoundTD").value == "false") {
		document.getElementById(prefix_component + "nofoundTD").value = "true";
	...
	document.getElementById(prefix_component + '__matiepdon').disabled = (document.getElementById(prefix_component + '__matiepdon').value != "");
...
function displayAfterGhiNhan(){
	...
	document.getElementById("div_huyphieu").style.display = "block";
	//phuc.lc : 18-01-2011 : End Fix bug 1360
...
function resetAfterNhapMoiXuatHang(){
	...
	document.getElementById(prefix_component + '__matiepdon').disabled = false;
}
